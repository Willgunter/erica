...
class LessonScene(Scene):
    def construct(self):
        ...
        title = Text("Parabola's X-Intercepts and the Discriminant", font_size=52, color=BLUE_B, weight=BOLD)
        title.to_edge(UP)
        ...
        objective = Text("Understand and apply parabola's x-intercepts and the discriminant", font_size=26, color=GREY_A)
        ...
        content = Text("When you write a quadratic equation as `y = ax^2 + bx + c`, its graph is a parabola. The solutions 'x' you find using th", font_size=25, color=GREY_A, line_spacing=0.9)
        ...
        content = Text("Review the flashcards for Parabola's X-Intercepts and the Discriminant. Try to explain each concept back in your own wor", font_size=25, color=GREY_A, line_spacing=0.9)
        content.next_to(step_title, DOWN, buff=0.5)
        frame = RoundedRectangle(corner_radius=0.18, width=12.2, height=2.3)
        frame.set_stroke(BLUE_D, width=2)
        frame.move_to(content)
