...
export async function POST(request: Request) {
  try {
    ...
    const targets = buildCandidateBaseUrls(FLASK_API_URL).map((base) => `${base}/api/lesson/generate`);
    const networkFailures: Array<{ target: string; message: string }> = [];
    const route404Targets: string[] = [];
    let lastErrorStatus = 502;
    let lastErrorData: JsonResponse = { error: "Failed to generate lesson" };
    ...
    if (route404Targets.length > 0 && route404Targets.length === targets.length) {
      return NextResponse.json(
        {
          error: "Lesson generation endpoint not found",
          details: `Connected to backend but /api/lesson/generate was not found. Tried: ${targets.join(", ")}`,
      ...
      return NextResponse.json(
        {
          ...
          details: `No backend reachable for lesson generation. Tried: ${targets.join(", ")}`,
